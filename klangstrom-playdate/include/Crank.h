...
public:
    ...
    void reset(const float crank_angle) {
    ...
    void update(const float crank_angle) {
        const float currentAngle = crank_angle;
        ...
        const float triggerAngle     = FULL_ROTATION / static_cast<float>(fNumEventsPerRotation);
        const float absTotalRotation = fabs(totalRotation);

        for (int i = 0; i < fNumEventsPerRotation; ++i) {
            const float eventAngle = triggerAngle * (static_cast<float>(i) + 1.0f);
